    ...
    (async () => {
        ...
        async function fetchData() {
            try {
                const [charRes, suitRes, echoRes, metaRes, suitMetaRes, entryStatsRes, entryCoefRes, echoMetaRes] = await Promise.all([
                    fetch(`../assets/characters.txt`),
                    fetch(`../assets/suit.txt`),
                    fetch(`../assets/echo.json`),
                    fetch(`../assets/imgs/char/metadata.json`),
                    fetch(`../assets/imgs/suit/metadata.json`),
                    fetch(`../assets/config/entry_stats.yml`),
                    fetch(`../assets/config/entry_coef.yml`),
                    fetch(`../assets/imgs/echo/metadata.json`)
                ]);
        ...
        function updateAvatar(selectedCharEn) {
            ...
            if (character && character.file) {
                ...
                const imgPath = `../assets/imgs/char/${character.file}`;
                if (!avatarImg) {
        ...
        function updateSuitIcon(selectedSuit) {
            ...
            if (selectedSuit) {
                ...
                if (suit && suit.file) {
                    const imgPath = `../assets/imgs/suit/${suit.file}`;
                    if (!suitIcon) {
        ...
        function renderScannedEchos(selectedOriginalIndex = -1) {
            ...
            sortedProfiles.forEach(item => {
                ...
                if (echoMeta && echoMeta.file) {
                    ...
                    img.src = `../assets/imgs/echo/${echoMeta.file}`;
